...
export const BlogPostDetail = ({ post, content, onBack }: BlogPostDetailProps) => {
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-20 right-4 z-40 flex items-center gap-3 transition-all duration-300 ${scrollProgress > 5 ? "opacity-100 translate-x-0" : "opacity-0 translate-x-4"}`}
      >
        {/* Time remaining */}
      ...
      {/* Floating Action Bar */}
      <div
        className={`fixed bottom-24 left-1/2 -translate-x-1/2 z-40 transition-all duration-500 ${scrollProgress > 10 && scrollProgress < 95 ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 pointer-events-none"}`}
      >
        <div className="flex items-center gap-1 px-2 py-1.5 rounded-full bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl border border-slate-200/60 dark:border-slate-700/60 shadow-2xl">
          ...
          <button
            onClick={handleLike}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full transition-all duration-200 ${liked ? "bg-pink-100 dark:bg-pink-900/30 text-pink-600 dark:text-pink-400" : "hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-500 dark:text-slate-400"}`}
          >
          ...
          <div className="p-6 sm:p-8 relative">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={copyLink}
                  className={`p-2.5 rounded-full transition-all duration-200 hover:scale-110 hover:shadow-lg ${copied ? "bg-green-500 text-white" : "bg-violet-500 text-white"}`}
                  title="Copy link"
                >
      ...
      <button
        onClick={scrollToTop}
        className={`fixed bottom-6 right-6 z-50 group transition-all duration-500 ${showBackToTop ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8 pointer-events-none"}`}
        title="Back to top"
      >
