  ...
  return <section className="py-20 relative bg-white/30 dark:bg-gray-900/30 backdrop-blur-3xl border-t border-white/30 dark:border-white/20">
      ...
      <div className="container mx-auto px-4 space-y-16 relative z-10">
        ...
        <div className="relative">
          <Carousel setApi={setApi} className="w-full max-w-7xl mx-auto" opts={{
          align: "start",
          loop: true,
          skipSnaps: false,
          dragFree: false
          ...
          stopOnMouseEnter: true
            ...
            <CarouselContent className="-ml-0 md:-ml-0">
              {caseStudies.map((study, index) => <CarouselItem key={index} className="pl-0 md:pl-0 basis-full md:basis-1/2 lg:basis-1/2">
                  <div className="h-full p-4">
                    <div onClick={() => handleCardClick(study.link)} className="bg-white/20 dark:bg-gray-800/20 backdrop-blur-xl rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 group h-full flex flex-col cursor-pointer transform hover:scale-[1.02] border border-white/30 dark:border-gray-700/30 aspect-[4/3]">
                      {/* Image Section - 75% */}
                      <div className="relative flex-[3] overflow-hidden">
                        <img src={study.image} alt={study.title} className="w-full h-full object-cover object-top" />
          ...
          <div className="flex justify-center mt-8 gap-2">
            {caseStudies.map((_, index) => <button key={index} onClick={() => scrollToSlide(index)} className={`w-3 h-3 rounded-full transition-all duration-300 ${current === index ? "bg-blue-600 dark:bg-blue-400 scale-125" : "bg-gray-300 dark:bg-gray-600 hover:bg-gray-400 dark:hover:bg-gray-500"}`} aria-label={`Go to slide ${index + 1}`} />)}
          </div>
        </div>
